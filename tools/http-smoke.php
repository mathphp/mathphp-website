...
try {
    ...
    $optionalChecks = [
        ...
        'numerical-parabolic-pde-2d' => ['path' => '/?api=pde-parabolic-2d-numeric', 'payload' => ['equation' => 'u_t = alpha*u_xx + beta*u_yy + gamma*u_xy', 'initialProfile' => '1', 'leftBoundary' => '1', 'rightBoundary' => '1', 'bottomBoundary' => '1', 'topBoundary' => '1', 'boundaryConditions' => ['left' => ['type' => 'neumann', 'value' => '0']], 'known' => ['alpha' => 0.1, 'beta' => 0.1, 'gamma' => 0.05], 'firstPoints' => 5, 'secondPoints' => 5, 'timeSteps' => 1, 'targetTime' => 0.02], 'success' => static fn (array $body): bool => ($body['analysis']['status'] ?? null) === 'solved' && ($body['analysis']['solution']['method'] ?? null) === 'explicit-2d-parabolic-finite-difference' && ($body['analysis']['solution']['boundaryConditions']['left']['type'] ?? null) === 'neumann' && abs((float) ($body['analysis']['solution']['final']['values'][2][2] ?? NAN) - 1.0) < 1e-10, 'unavailable' => 'explain.unavailable'],
