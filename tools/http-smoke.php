...
try {
    ...
    $optionalChecks = [
        ...
        'numerical-pde' => ['path' => '/?api=pde-numeric', 'payload' => ['equation' => 'u_t = k*u_xx', 'initialProfile' => '0', 'leftBoundary' => '0', 'rightBoundary' => '0', 'known' => ['k' => 0.2], 'boundaryConditions' => ['left' => ['type' => 'neumann', 'value' => '0'], 'right' => ['type' => 'dirichlet', 'value' => '0']], 'spacePoints' => 17, 'timeSteps' => 8], 'success' => static fn (array $body): bool => ($body['analysis']['status'] ?? null) === 'solved' && ($body['analysis']['solution']['method'] ?? null) === 'explicit-finite-difference' && ($body['analysis']['solution']['boundaryConditions']['left']['type'] ?? null) === 'neumann' && abs((float) ($body['analysis']['solution']['final']['values'][8] ?? NAN)) < 1e-10, 'unavailable' => 'explain.unavailable'],
