...
try {
    ...
    $optionalChecks = [
        ...
        'numerical-parabolic-pde-2d' => ['path' => '/?api=pde-parabolic-2d-numeric', 'payload' => ['equation' => 'u_t = alpha*u_xx + beta*u_yy', 'initialProfile' => '1', 'leftBoundary' => '1', 'rightBoundary' => '1', 'bottomBoundary' => '1', 'topBoundary' => '1', 'boundaryConditions' => ['left' => ['type' => 'neumann', 'value' => '0']], 'known' => ['alpha' => 0.1, 'beta' => 0.1], 'firstPoints' => 7, 'secondPoints' => 7, 'timeSteps' => 2], 'success' => static fn (array $body): bool => ($body['analysis']['status'] ?? null) === 'solved' && ($body['analysis']['solution']['method'] ?? null) === 'explicit-2d-parabolic-finite-difference' && ($body['analysis']['solution']['boundaryConditions']['left']['type'] ?? null) === 'neumann' && abs((float) ($body['analysis']['solution']['final']['values'][3][3] ?? NAN) - 1.0) < 1e-10, 'unavailable' => 'explain.unavailable'],
