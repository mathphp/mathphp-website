...
function renderLayout(string $title, string $content, string $active): string
{
    ...
    return '<!doctype html><html lang="en"><head><meta charset="utf-8">'
        ...
        . '<main>' . $content . '</main>'
        . '<footer class="site-footer"><div class="footer-brand"><span class="brand-mark" aria-hidden="true">M</span><div><strong>MathPHP</strong><small>Deterministic mathematics for PHP.</small></div></div><div class="footer-links"><div><b>Build</b><a href="?page=getting-started">Installation</a><a href="?page=docs">Guides</a><a href="?page=api">API reference</a></div><div><b>Explore</b><a href="?page=playground">Playground</a><a href="?page=visuals">Visuals</a><a href="?page=packages">Packages</a></div><div><b>Community</b><a href="https://github.com/mathphp/mathphp">GitHub</a><a href="https://github.com/mathphp/mathphp/issues">Issues</a><a href="?page=pricing">Sponsor access</a></div></div><a class="footer-sponsor" href="?page=pricing"><span>✦</span><strong>Support MathPHP</strong><small>Keep the core open and the add-ons maintained →</small></a><div class="footer-bottom"><span>© 2026 MathPHP</span><span>Open core · explicit contracts</span></div></footer>'
...
function renderGuide(string $guide): string
{
    ...
    $guides = [
        ...
        'explaining-equations' => ['Explaining add-on', 'Equation analysis', 'Describe what is known, what is unknown, and which transformations are safe to show to a learner.', '<h2>Make unknowns explicit.</h2><p><code>EquationAnalyzer</code> returns a serializable analysis rather than pretending every equation has a closed-form solution. Use it to show the original equation, known values, unknown symbols, and an honest status.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> EquationAnalyzer())-&gt;analyze(<span class="code-string">\'x^2 + 1 = 5\'</span>);<br><span class="code-comment">// render $analysis-&gt;toArray() in your UI</span></code></pre><div class="guide-note"><strong>Good fit:</strong><span>Worked examples, tutoring hints, and validation previews where partial solving is more useful than a guessed answer.</span></div>'],
        'explaining-systems' => ['Explaining add-on', 'Linear systems', 'Turn a set of equations into a structured explanation of variables, constraints, and solution status.', '<h2>Explain the system as a system.</h2><p><code>SystemAnalyzer</code> keeps each equation and its unknowns together. This lets a UI explain substitution or elimination without parsing a user-facing string twice.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> SystemAnalyzer())-&gt;analyze([<span class="code-string">\'2*x + 3*y = 8\'</span>, <span class="code-string">\'x - y = 1\'</span>]);</code></pre><p>Use the returned model for a table, a guided exercise, or a compact API response. Keep numerical solving and teaching copy as separate concerns.</p>'],
        'explaining-matrices' => ['Explaining add-on', 'Matrix analysis', 'Inspect matrix dimensions, entries, and derived metadata before presenting a heatmap or operation trace.', '<h2>Dimensions are part of the explanation.</h2><p><code>MatrixAnalyzer</code> accepts nested numeric arrays and returns a stable model for rows, columns, shape, and values. Invalid or ragged input fails early with an input error.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> MatrixAnalyzer())-&gt;analyze([[1, 2], [3, 4]]);</code></pre><div class="guide-note"><strong>Pair it with:</strong><span><a href="?page=visuals-rendering">Visuals rendering</a> for an accessible matrix view.</span></div>'],
        'explaining-calculus' => ['Explaining add-on', 'Calculus analysis', 'Represent derivatives and antiderivatives with the expression, variable, operation, and result metadata intact.', '<h2>Show the operation and its assumptions.</h2><p><code>CalculusAnalyzer</code> keeps derivative and integral requests explicit. That makes it possible to show the transformation, the variable of differentiation, and numerical checks in separate UI elements.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> CalculusAnalyzer())-&gt;analyze(<span class="code-string">\'x^2 + 3*x\'</span>, <span class="code-string">\'derivative\'</span>, <span class="code-string">\'x\'</span>);</code></pre><p>For sampled curves, hand the model to <a href="?page=visuals-plots">Plotter</a> and label numerical approximations clearly.</p>'],
        'explaining-area' => ['Explaining add-on', 'Area analysis', 'Explain numerical area under a curve with interval, sampling, and approximation metadata.', '<h2>Make approximation visible.</h2><p><code>AreaAnalyzer</code> is useful when the learner needs to see the interval and method, not only the final number. Preserve sample counts and endpoints in your presentation.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> AreaAnalyzer())-&gt;analyze(<span class="code-string">\'x^2\'</span>, <span class="code-string">\'x\'</span>, 0, 1);</code></pre><p>Combine the model with a shaded area representation and an accessible textual summary.</p>'],
        'explaining-roots' => ['Explaining add-on', 'Root finding', 'Show a bracket, convergence, and the resulting approximation rather than presenting a root as magic.', '<h2>Explain convergence.</h2><p><code>RootAnalyzer</code> exposes the interval and numerical result so a UI can show why a root is plausible and when the bracket is invalid.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> RootAnalyzer())-&gt;analyze(<span class="code-string">\'x^2 - 2\'</span>, <span class="code-string">\'x\'</span>, 0, 2);</code></pre><div class="guide-note"><strong>Product hint:</strong><span>Always label approximate values and preserve the bracket in exports.</span></div>'],
