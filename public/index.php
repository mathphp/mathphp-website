...
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>parabolic fields.</em></h1><p>Integrate bounded heat, advection, and diffusion equations on a rectangular grid with explicit edge conditions, effective time steps, and stability evidence.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalParabolicPdeAnalyzer</code> accepts <code>u_t = F(x,y,t,u,u_x,u_y,u_xx,u_yy,u_xy)</code> when the spatial derivative terms are affine, diffusion is non-negative, and the principal part is positive-semidefinite.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalParabolicPdeAnalyzer())-&gt;analyze(<span class="code-string">\'u_t = vx*u_x + vy*u_y + alpha*u_xx + beta*u_yy + gamma*u_xy\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, known: [<span class="code-string">\'vx\'</span> =&gt; 0.2, <span class="code-string">\'vy\'</span> =&gt; -0.1, <span class="code-string">\'alpha\'</span> =&gt; 0.1, <span class="code-string">\'beta\'</span> =&gt; 0.1, <span class="code-string">\'gamma\'</span> =&gt; 0.05]);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Sample the initial field and apply the four positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Transport & diffuse</h3><p>Apply centered gradient stencils, five-point diffusion, and diagonal mixed-derivative coupling at each combined CFL-controlled substep.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return time-stamped grids, normalized boundary metadata, step size, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Nonlinear derivative terms, backward diffusion, non-positive-semidefinite operators, periodic/nonlocal edges, unstable grids, higher dimensions, and symbolic closed forms remain unsupported. A completed grid is an approximation, never a proof of every PDE solution.</span></div></div></section>';
}
...
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>wave fields.</em></h1><p>Propagate a bounded wave across a rectangular grid with initial displacement, initial velocity, explicit edge conditions, and CFL evidence.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalWavePde2DAnalyzer</code> accepts <code>u_tt = F(x,y,t,u,u_xx,u_yy,u_xy)</code> when the spatial derivative terms are affine, wave coefficients are non-negative, and the principal part is positive-semidefinite.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalWavePde2DAnalyzer())-&gt;analyze(<span class="code-string">\'u_tt = c^2*(u_xx + u_yy) + gamma*u_xy\'</span>, <span class="code-string">\'sin(pi()*x)*sin(pi()*y)\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, known: [<span class="code-string">\'c\'</span> =&gt; 1, <span class="code-string">\'gamma\'</span> =&gt; 0.1], boundaryConditions: [<span class="code-string">\'left\'</span> =&gt; [<span class="code-string">\'type\'</span> =&gt; <span class="code-string">\'neumann\'</span>, <span class="code-string">\'value\'</span> =&gt; <span class="code-string">\'0\'</span>]]);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Sample displacement and velocity over the rectangular grid and apply positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Propagate</h3><p>Use the centered leapfrog update with diagonal mixed-derivative coupling, a first-step velocity correction, and a conservative 2D CFL bound.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return time-stamped grids, normalized boundary metadata, effective time step, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Nonlinear derivative terms, periodic/nonlocal edges, unstable coefficients, higher dimensions, and symbolic closed forms remain unsupported. A completed grid is an approximation, never a proof.</span></div></div></section>';
}
...
function handleCapabilitiesRequest(): never
{
    ...
    $capabilities = [
        ...
        ['id' => 'numerical-wave-pde-2d', 'endpoint' => '?api=pde-wave-2d-numeric', 'input' => 'bounded 2D wave PDE with affine u_xx/u_yy/u_xy terms, initial displacement/velocity, per-edge Dirichlet/Neumann/Robin conditions, CFL-limited time grid', 'visualKinds' => ['pde-wave-2d'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
