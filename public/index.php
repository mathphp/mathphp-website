...
function renderGuide(string $guide): string
{
    ...
    $guides = [
        ...
        'explaining-equations' => ['Explaining add-on', 'Equation analysis', 'Describe what is known, what is unknown, and which transformations are safe to show to a learner.', '<h2>Make unknowns explicit.</h2><p><code>EquationAnalyzer</code> returns a serializable analysis rather than pretending every equation has a closed-form solution. Its first quadratic rule uses an explicit coefficient form, so the input grammar stays unambiguous.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> EquationAnalyzer())-&gt;analyze(<span class="code-string">\'1*x^2 + 0*x + 1 = 5\'</span>);<br><span class="code-comment">// render $analysis-&gt;toArray() in your UI</span></code></pre><div class="guide-note"><strong>Good fit:</strong><span>Worked examples, tutoring hints, and validation previews where partial solving is more useful than a guessed answer.</span></div>'],
