...
function renderGuide(string $guide): string
{
    ...
    $guides = [
        ...
        'explaining-systems' => ['Explaining add-on', 'Linear systems', 'Turn a set of equations into a structured explanation of variables, constraints, and solution status.', '<h2>Explain the system as a system.</h2><p><code>SystemAnalyzer</code> accepts two equations in one semicolon- or newline-delimited string. Coefficients may be explicit or implicit, so familiar forms such as <code>2x + 3y = 8; x - y = 1</code> work directly. It keeps both constraints and the solution status together so a UI does not need to parse the text twice.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> SystemAnalyzer())-&gt;analyze(<span class="code-string">\'2x + 3y = 8; x - y = 1\'</span>);</code></pre><p>Use the returned model for a table, a guided exercise, or a compact API response. Keep numerical solving and teaching copy as separate concerns.</p>'],
