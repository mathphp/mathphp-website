...
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>elliptic fields.</em></h1><p>Solve bounded Laplace, Poisson, and rotated-coordinate fields on a rectangular grid with Gauss–Seidel evidence and explicit edge conditions.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalEllipticPdeAnalyzer</code> accepts residuals affine in <code>u_xx</code>, <code>u_yy</code>, and <code>u_xy</code>, such as <code>u_xx + u_yy + 0.25*u_xy = 0</code>, when the principal part remains elliptic.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalEllipticPdeAnalyzer())-&gt;analyze(<span class="code-string">\'u_xx + u_yy + 0.25*u_xy = 0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, boundaryConditions: [<span class="code-string">\'left\'</span> =&gt; [<span class="code-string">\'type\'</span> =&gt; <span class="code-string">\'neumann\'</span>, <span class="code-string">\'value\'</span> =&gt; <span class="code-string">\'0\'</span>]]);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Build the rectangular field and apply positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Iterate</h3><p>Update interior nodes with a coefficient-aware Gauss–Seidel stencil, including the diagonal mixed-derivative coupling, until change and residual tolerances agree.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return snapshots, residual metrics, normalized boundary metadata, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Nonlinear or non-elliptic operators, periodic/nonlocal edges, singular Robin coefficients, higher dimensions, and symbolic closed forms remain unsupported. Convergence is a bounded numerical result, not a uniqueness proof.</span></div></div></section>';
}
