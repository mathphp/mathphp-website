...
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>parabolic fields.</em></h1><p>Integrate bounded heat, advection, diffusion, and directly evaluated nonlinear spatial operators on a rectangular grid with explicit edge conditions and stability evidence.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalParabolicPdeAnalyzer</code> accepts <code>u_t = F(x,y,t,u,u_x,u_y,u_xx,u_yy,u_xy)</code> with affine transport/diffusion terms or nonlinear derivative combinations whose stencil samples remain finite.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalParabolicPdeAnalyzer())-&gt;analyze(<span class="code-string">\'u_t = 0.1*u_xx^2 + 0.05*u_xy\'</span>, <span class="code-string">\'sin(3.141592653589793*x)*sin(3.141592653589793*y)\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, firstPoints: 25, secondPoints: 25, timeSteps: 100, targetTime: 0.1);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Sample the initial field and apply positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Evaluate</h3><p>Use centered gradients, second derivatives, and mixed stencils. Nonlinear operators are evaluated directly with local sensitivity estimates.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return time-stamped grids, normalized boundary metadata, operator mode, step size, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Backward diffusion, non-positive-semidefinite affine operators, periodic/nonlocal edges, unstable grids, higher dimensions, and symbolic closed forms remain unsupported. A completed grid is an approximation, never a proof.</span></div></div></section>';
}

function renderEllipticPdeGuide(): string
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>elliptic fields.</em></h1><p>Solve bounded Laplace, Poisson, rotated-coordinate, and semilinear value fields on a rectangular grid with Gauss–Seidel evidence and explicit edge conditions.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalEllipticPdeAnalyzer</code> accepts residuals affine in <code>u_xx</code>, <code>u_yy</code>, and <code>u_xy</code>, including value terms such as <code>u^3</code>, when the principal part remains elliptic.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalEllipticPdeAnalyzer())-&gt;analyze(<span class="code-string">\'u_xx + u_yy + u^3 = 0\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Build the rectangular field and apply positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Iterate</h3><p>Update interior nodes with a coefficient-aware Gauss–Seidel stencil, re-evaluating value-dependent terms with Picard updates and including diagonal mixed-derivative coupling.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return snapshots, residual metrics, normalized boundary metadata, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Nonlinear spatial derivative operators, non-elliptic operators, periodic/nonlocal edges, singular Robin coefficients, higher dimensions, and symbolic closed forms remain unsupported. Convergence is a bounded numerical result, not a uniqueness proof.</span></div></div></section>';
}

function renderCoupledParabolicPdeGuide(): string
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Coupled<br><em>reaction–transport.</em></h1><p>Integrate multiple interacting fields on one bounded one-dimensional grid with per-field advection, diffusion, reaction coupling, and stability evidence.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalCoupledParabolicPdeAnalyzer</code> accepts one time-derivative equality per field, with reaction coupling, affine first and second spatial derivatives, and independent edge conditions for every field.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalCoupledParabolicPdeAnalyzer())-&gt;analyze(<span class="code-string">\'u_t = a*u_x + u_xx + v; v_t = b*v_x + v_xx - u\'</span>, [<span class="code-string">\'u\'</span>, <span class="code-string">\'v\'</span>], [<span class="code-string">\'u\'</span> =&gt; <span class="code-string">\'sin(pi()*x)\'</span>, <span class="code-string">\'v\'</span> =&gt; <span class="code-string">\'0\'</span>], [<span class="code-string">\'u\'</span> =&gt; <span class="code-string">\'0\'</span>, <span class="code-string">\'v\'</span> =&gt; <span class="code-string">\'0\'</span>], [<span class="code-string">\'u\'</span> =&gt; <span class="code-string">\'0\'</span>, <span class="code-string">\'v\'</span> =&gt; <span class="code-string">\'0\'</span>], known: [<span class="code-string">\'a\'</span> =&gt; 0.2, <span class="code-string">\'b\'</span> =&gt; -0.1]);</code></pre><h2>How the system is integrated</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Share a grid</h3><p>Every field uses the same spatial points and time levels, so coupling is evaluated consistently.</p></div><div><span class="feature-number">02</span><h3>Transport, diffuse & react</h3><p>Apply centered gradients and Laplacians, then evaluate each field’s reaction terms from the same state.</p></div><div><span class="feature-number">03</span><h3>Apply edge rules</h3><p>Keep positional Dirichlet defaults or set per-field Dirichlet, Neumann, and Robin conditions on the left and right edges.</p></div><div><span class="feature-number">04</span><h3>Report evidence</h3><p>Return per-field snapshots, normalized boundary metadata, effective step size, CFL metadata, and explicit status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Nonlinear derivative terms, backward diffusion, higher-dimensional systems, and symbolic/global PDE solutions remain unsupported. A completed grid is an approximation, never a proof.</span></div></div></section>';
}

function renderWavePde2DGuide(): string
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>wave fields.</em></h1><p>Propagate bounded wave fields with affine or directly evaluated nonlinear spatial operators, explicit edge conditions, and CFL evidence.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalWavePde2DAnalyzer</code> accepts <code>u_tt = F(x,y,t,u,u_xx,u_yy,u_xy)</code>. Affine operators require non-negative wave coefficients and a positive-semidefinite principal part; nonlinear combinations are sampled directly when finite.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalWavePde2DAnalyzer())-&gt;analyze(<span class="code-string">\'u_tt = u_xx^2 + u_yy\'</span>, <span class="code-string">\'sin(3.141592653589793*x)*sin(3.141592653589793*y)\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, <span class="code-string">\'0\'</span>, firstPoints: 25, secondPoints: 25, timeSteps: 100, targetTime: 0.1);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Sample displacement and velocity over the rectangular grid and apply positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Propagate</h3><p>Use a centered leapfrog update with diagonal mixed-derivative coupling. Nonlinear operators receive local sensitivity estimates for the CFL guard.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return time-stamped grids, normalized boundary metadata, operator mode, effective time step, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Periodic/nonlocal edges, unstable affine coefficients, higher dimensions, and symbolic closed forms remain unsupported. A completed grid is an approximation, never a proof.</span></div></div></section>';
}
...
function renderGuide(string $guide): string
{
    ...
    if ($guide === 'explaining-pde-elliptic-2d') {
        return renderEllipticPdeGuide();
    }
    ...
    $guides = [
        ...
        'explaining-equations' => ['Explaining add-on', 'Equation analysis', 'Describe what is known, what is unknown, and which transformations are safe to show to a learner.', '<h2>Make unknowns explicit.</h2><p><code>EquationAnalyzer</code> returns a serializable analysis rather than pretending every equation has a closed-form solution. Use its closed-form rules for linear, quadratic, and power forms, or use <code>NumericalEquationAnalyzer</code> for any single-variable equality that Core can evaluate.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> EquationAnalyzer())-&gt;analyze(<span class="code-string">\'1*x^2 + 0*x + 1 = 5\'</span>);<br><span class="code-comment">// render $analysis-&gt;toArray() in your UI</span></code></pre><pre class="code-block"><code><span class="code-keyword">$numeric</span> = (<span class="code-keyword">new</span> NumericalEquationAnalyzer())-&gt;analyze(<span class="code-string">\'sin(x) = x / 2\'</span>, <span class="code-string">\'x\'</span>, -8, 8);<br><span class="code-comment">// roots: approximately -1.89549, 0, 1.89549</span></code></pre><h2>Conditional branches stay explicit</h2><p><code>PiecewiseEvaluator</code> handles bounded <code>piecewise(condition: value; otherwise: value)</code> and <code>if(condition, true, false)</code> expressions. It evaluates conditions top-to-bottom, reports the selected branch and steps, and evaluates only that branch.</p><pre class="code-block"><code><span class="code-keyword">$piece</span> = (<span class="code-keyword">new</span> PiecewiseEvaluator())-&gt;explain(<span class="code-string">\'piecewise(x &lt; 0: -x; otherwise: x)\'</span>, [<span class="code-string">\'x\'</span> =&gt; -3]);<br><span class="code-comment">// value: 3; branch: 1</span></code></pre><h2>Limits and chained bounds</h2><p><code>LimitAnalyzer</code> estimates one- or two-sided finite limits from geometric samples and preserves non-convergence as <code>partial</code>. <code>InequalityAnalyzer</code> intersects chained relations such as <code>1 &lt; x ≤ 3</code> while retaining open and closed endpoints.</p><h2 id="pde">Bounded one-dimensional PDEs</h2><p><code>NumericalPdeAnalyzer</code> integrates <code>u_t = F(x,t,u,u_x,u_xx)</code> on a finite grid with centered advection and diffusion stencils, a direct nonlinear-operator fallback, Dirichlet, Neumann, or Robin boundary conditions, and a local sensitivity stability guard. It returns <code>pde-heatmap</code> snapshots. Undefined operators and higher-dimensional or symbolic PDEs remain explicitly unsupported.</p><div class="guide-note"><strong>Numerical scope:</strong><span>Finite intervals and sampled bisection provide approximate roots and limits; <code>partial</code> remains possible for domain gaps, tangent roots, side disagreement, singular boundary coefficients, or roots outside the interval. These analyzers expose evidence, not universal symbolic proofs.</span></div>'],
        'explaining-systems' => ['Explaining add-on', 'Linear & nonlinear systems', 'Turn a set of equations into structured constraints, local numerical roots, and honest convergence evidence.', '<h2>Choose the right system solver.</h2><p><code>SystemAnalyzer</code> handles exact affine constraints such as <code>2x + 3y = 8; x - y = 1</code>. For nonlinear multivariable equations, <code>NonlinearSystemAnalyzer</code> uses finite-difference Jacobians with damped Newton or Gauss–Newton updates, backtracking when a full step increases the residual.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NonlinearSystemAnalyzer())-&gt;analyze(<span class="code-string">\'x^2 + y^2 = 5; x - y = 1\'</span>, [<span class="code-string">\'x\'</span>, <span class="code-string">\'y\'</span>], [<span class="code-string">\'x\'</span> =&gt; 1.5, <span class="code-string">\'y\'</span> =&gt; 0.5]);</code></pre><p>Use the returned iterate history and residual norms to show convergence. A local numerical root is not a proof that every solution has been found.</p>'],
    ...
    $addonNav = ['explaining-pde-2d' => 'Explaining · 2D Parabolic PDE', 'explaining-pde-elliptic-2d' => 'Explaining · 2D Elliptic PDE', 'explaining-pde-coupled' => 'Explaining · Coupled Parabolic PDE', 'explaining-pde-wave-2d' => 'Explaining · 2D Wave PDE'] + $addonNav;
    $nav = '';
...
function handlePiecewiseRequest(): never
{
    header('Content-Type: application/json; charset=utf-8');
    if (!class_exists('MathPHP\\Explaining\\PiecewiseEvaluator')) {
        echo json_encode(['ok' => false, 'code' => 'explain.unavailable', 'message' => 'The private mathphp-explaining package is not installed on this deployment.'], JSON_THROW_ON_ERROR);
        exit;
    }
    $payload = json_decode((string) file_get_contents('php://input'), true);
    ...
    $rawVariables = is_array($payload) && is_array($payload['variables'] ?? null) ? $payload['variables'] : [];
    try {
        $result = (new \MathPHP\Explaining\PiecewiseEvaluator())->explain($expression, normalizeVariables($rawVariables));
        echo json_encode(['ok' => true, 'result' => $result->toArray()], JSON_THROW_ON_ERROR);
    } catch (InvalidArgumentException|\DomainException $error) {
        echo json_encode(['ok' => false, 'code' => 'piecewise.invalid', 'message' => $error->getMessage()], JSON_THROW_ON_ERROR);
    }
    exit;
}
...
function handleCapabilitiesRequest(): never
{
    ...
    $capabilities = [
        ...
        ['id' => 'numerical-pde', 'endpoint' => '?api=pde-numeric', 'input' => 'bounded 1D parabolic PDE with affine or directly evaluated nonlinear u_x/u_xx operators, initial profile, Dirichlet/Neumann/Robin boundaries, finite grid/time limits', 'visualKinds' => ['pde-heatmap'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
        ...
        ['id' => 'numerical-elliptic-pde', 'endpoint' => '?api=pde-elliptic-numeric', 'input' => 'bounded 2D elliptic PDE with affine u_xx/u_yy/u_xy terms plus Picard-evaluated value terms, per-edge Dirichlet/Neumann/Robin conditions, finite grid and iteration limits', 'visualKinds' => ['differential-equation-elliptic-pde'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
        ['id' => 'numerical-wave-pde', 'endpoint' => '?api=pde-wave-numeric', 'input' => 'bounded 1D wave PDE with affine or directly evaluated nonlinear u_xx operators, initial displacement/velocity, per-edge Dirichlet/Neumann/Robin conditions, CFL-limited time grid', 'visualKinds' => ['pde-wave'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
        ['id' => 'numerical-wave-pde-2d', 'endpoint' => '?api=pde-wave-2d-numeric', 'input' => 'bounded 2D wave PDE with affine or directly evaluated nonlinear u_xx/u_yy/u_xy operators, initial displacement/velocity, per-edge Dirichlet/Neumann/Robin conditions, CFL-limited time grid', 'visualKinds' => ['pde-wave-2d'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
        ['id' => 'numerical-parabolic-pde-2d', 'endpoint' => '?api=pde-parabolic-2d-numeric', 'input' => 'bounded 2D parabolic PDE with affine u_x/u_y/u_xx/u_yy/u_xy advection–diffusion terms, initial field, per-edge Dirichlet/Neumann/Robin conditions, CFL-limited time grid', 'visualKinds' => ['pde-heatmap-2d'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
        ['id' => 'numerical-coupled-parabolic-pde', 'endpoint' => '?api=pde-coupled-parabolic-numeric', 'input' => 'bounded coupled 1D parabolic system with per-field affine or directly evaluated nonlinear u_x/u_xx operators, profiles, Dirichlet/Neumann/Robin edges, CFL-limited time grid', 'visualKinds' => ['pde-system-heatmap'], 'requiredPackages' => ['core', 'explaining'], 'available' => $state['core'] && $state['optional']['explaining']],
