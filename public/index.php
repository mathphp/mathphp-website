...
{
    return '<section class="page-intro wrap"><div class="eyebrow"><span class="eyebrow-dot"></span>Explaining add-on</div><h1>Two-dimensional<br><em>parabolic fields.</em></h1><p>Integrate bounded heat and diffusion equations on a rectangular grid with explicit edge conditions, effective time steps, and stability evidence.</p></section><section class="guide-page wrap"><div class="guide-content guide-content-wide"><a class="guide-back" href="?page=docs">← All documentation</a><h2>What it supports</h2><p><code>NumericalParabolicPdeAnalyzer</code> accepts <code>u_t = F(x,y,t,u,u_xx,u_yy)</code> when the spatial second-derivative terms are affine with non-negative diffusion coefficients.</p><pre class="code-block"><code><span class="code-keyword">$analysis</span> = (<span class="code-keyword">new</span> NumericalParabolicPdeAnalyzer())-&gt;analyze(<span class="code-string">\'u_t = alpha*u_xx + beta*u_yy\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, <span class="code-string">\'1\'</span>, known: [<span class="code-string">\'alpha\'</span> =&gt; 0.1, <span class="code-string">\'beta\'</span> =&gt; 0.1], boundaryConditions: [<span class="code-string">\'left\'</span> =&gt; [<span class="code-string">\'type\'</span> =&gt; <span class="code-string">\'neumann\'</span>, <span class="code-string">\'value\'</span> =&gt; <span class="code-string">\'0\'</span>]]);</code></pre><h2>How the numerical contract works</h2><div class="guide-grid"><div><span class="feature-number">01</span><h3>Initialize</h3><p>Sample the initial field and apply the four positional Dirichlet defaults or configured edge conditions.</p></div><div><span class="feature-number">02</span><h3>Diffuse</h3><p>Apply the five-point explicit stencil to each interior node at every CFL-controlled substep.</p></div><div><span class="feature-number">03</span><h3>Report</h3><p>Return time-stamped grids, normalized boundary metadata, step size, and explicit <code>solved</code>/<code>partial</code> status.</p></div></div><div class="guide-note"><strong>Scope is intentional:</strong><span>Mixed or nonlinear derivative terms, backward diffusion, periodic/nonlocal edges, unstable grids, higher dimensions, and symbolic closed forms remain unsupported. A completed grid is an approximation, never a proof of every PDE solution.</span></div></div></section>';
}
